...
class GeomCA:
    def __init__(self, R, E, param_dict, load_existing=False, **kwargs):
        ...
        if load_existing:
            ...
            self.sparsify_points(
                R,
                param_dict["reduceR"],
                E,
                param_dict["reduceE"],
                param_dict["log_reduced"],
                param_dict["sparsify"],
                param_dict["n_Rsamples"],
                param_dict["n_Esamples"],
            )
    ...
    def log(self, msg, var=None, sub=False, nl=False):
        ...
        """
        log_msg = "{0}{1}{2}{3}".format("\n\n" if nl else "", " - " if sub else "", msg, ": " + str(var) if var != None else "")
    ...
    def log_to_txt(self):
        ...
        with open(path, "w") as f:
            ...
            f.writelines(
                [
                    "c(G{0}): {1:.2f}, q(G{0}): {2:.2f}, |G{0}^R|_v: {3:<4}, |G{0}^E|_v: {4:<4}, |G{0}|_v: {5:<4} \n".format(
                        comp_id,
                        round(v["comp_consistency"], 2),
                        round(v["comp_quality"], 2),
                        len(v["Ridx"]),
                        len(v["Eidx"]),
                        len(v["Ridx"]) + len(v["Eidx"]),
                    )
    ...
    def sparsify_points(self, R, reduceR, E, reduceE, log_reduced, sparsify, n_Rsamples=None, n_Esamples=None):
        """
    ...
    def get_sparse_pnts(self, pnts, type):
        ...
        sparse_pnts = np.array(gudhi.subsampling.sparsify_point_set(points=pnts, min_squared_dist=self.delta**2))
        fn_end = timer()
    ...
    def get_epsilon_edges(self):
        ...
        fn_start = timer()
        vrc = gudhi.RipsComplex(points=np.concatenate([self.sparseR, self.sparseE]), max_edge_length=self.epsilon)
    ...
    def plot_dist_hist(self, distances, filename, epsilon):
        ...
        plt.text((avg_dist - std_dist) * 1.1, max_ylim * 0.7, "Mean - std: {:.2f}".format(avg_dist - std_dist))

        plt.axvline(epsilon, color="k", linestyle="dashed", linewidth=1)
        plt.text(epsilon * 1.1, max_ylim * 0.5, "{1}th percentile: {0:.2f}".format(epsilon, self.Rdist_percentile))
    ...
    def plot_connected_components(self, figsize=(5, 5)):
        ...
        for i in range(n_components):
            ...
            plt.scatter(plotR[Ridxs, 0], plotR[Ridxs, 1], s=30, color=colors[i], alpha=1.0)
            plt.scatter(plotE[Eidxs, 0], plotE[Eidxs, 1], s=50, marker="X", color=colors[i], alpha=0.9)
    ...
    def plot_RE_components_quality(self, annotate_largest=True, min_comp_size=0, display_smaller=False, figsize=(10, 5)):
        """
        ...
        if annotate_largest:
            ...
            ax.annotate(
                ...
                xy=(0, self.comp_stats_logger[0]["comp_quality"] + 0.03),
                ha="center",
                va="bottom",
                color="k",
            )
        ...
        ax.legend(handles=legend_elements, ncol=len(legend_elements), loc="upper center", framealpha=0.5)
    ...
    def plot_RE_components_consistency(self, annotate_largest=True, min_comp_size=0, display_smaller=False, figsize=(10, 5)):
        """
        ...
        if annotate_largest:
            ...
            ax.annotate(
                round(largest_comp_size / total_n_pts, 2),
                xy=(0, self.comp_stats_logger[0]["comp_consistency"] + 0.03),
                ha="center",
                va="bottom",
                color="k",
            )
